...
const formatCompletionDuration = (minutes) => {
...
const CompleteTask = ({ data, onExplain, onDelete, theme = "dark" }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      updatedCounts.completed = Math.max((updatedCounts.completed || 0) - 1, 0);
  ...
  return (
    <div className="flex-shrink-0 w-full max-w-[280px] h-fit bg-gradient-to-br from-emerald-400 via-green-400 to-lime-400 rounded-xl p-4 shadow-lg shadow-black/10 text-gray-900 flex flex-col overflow-visible transition-all duration-300 ease-in-out hover:shadow-2xl hover:shadow-black/20">
      <div className="flex justify-between items-center flex-shrink-0">
        <h3 className="bg-white/80 text-green-700 px-3 py-1 rounded text-xs font-semibold">
          {data.category}
        </h3>
      ...
      <div className="overflow-visible">
        ...
        <p className="mt-2 text-xs font-semibold text-green-900/90">
          Completion time: {formatCompletionDuration(data.completionTime)}
        </p>
        <TaskAIInsight
      ...
      <div className="mt-3 flex flex-col gap-2 flex-shrink-0">
        <button className="bg-white text-green-700 w-full rounded-lg p-2 border border-green-700 hover:bg-green-50 hover:scale-[1.02] active:scale-[0.98] transition-all duration-200">
          Completed
        </button>
        <button
