          ...
          <BarChart data={chartData} layout="vertical">
            ...
            <Tooltip
              ...
              formatter={(value, name) => {
                if (name === "productivityScore")
                  return [`${value.toFixed(2)}`, "Productivity Score"];
                if (name === "onTimeRate")
                  return [`${value.toFixed(1)}%`, "On-Time Rate"];
                if (name === "avgCompletion")
                  return [`${value} min`, "Avg Completion"];
                if (name === "completedLast7")
                  return [value, "Completed Last 7 Days"];
                return [value, name];
              }}
            />
            <Legend />
        ...
        {chartData.slice(0, 3).map((entry, idx) => (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              <span
                className={`text-2xl font-bold ${idx === 0 ? "text-yellow-500" : theme === "dark" ? "text-gray-400" : "text-gray-600"}`}
              >
                #{entry.rank}
              </span>
              <h3
            ...
            <div
              className={`text-xs space-y-1 ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}
            >
              <div>
                Score:{" "}
                <span className="font-semibold">
                  {entry.productivityScore.toFixed(2)}
                </span>
              </div>
              <div>
                On-Time:{" "}
              ...
              <div>
                Avg Time:{" "}
                <span className="font-semibold">{entry.avgCompletion} min</span>
              </div>
              <div>
                Last 7 Days:{" "}
                <span className="font-semibold">
                  {entry.completedLast7} tasks
                </span>
              </div>
            </div>
          </div>
